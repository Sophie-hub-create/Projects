...
vid int primary key,
Versicherung varchar(60),
Typ varchar(60),
Zahlweise varchar(60),
Zahlbetrag dec (8,3),
Abschlussdatum date,
...
insert into versicherungen (vid, Versicherung, Typ,Zahlweise, Zahlbetrag,Abschlussdatum, Kündigungsdatum, Aktiv)
values (1, 'AXA', 'Hausrat', 'monatlich', 55.00, '01.10.15', '31.12.19', false),
(2, 'Generali','Haftpflicht', 'monatlich', 87.00, '30.06.16', null, true),
(3, 'HDI online', 'KFZ', 'jahr', 577.00, '01.11.16', null, true),
(4, 'Alianz', 'Unfall', 'quartal', 345.00, '01.10.15', '15.03.19', false),
(5, 'Thalanx', 'Unfall', 'quartal', 320.00, '01.03.19', null, true),
(6, 'Generali', 'Hausrat', 'monatlich', 45.00, '01.01.20', null, true);

select* from versicherungen order by abschlussdatum desc;
insert into versicherungen values(7, 'HUK', 'KFZ', 'jahr', 535.00, '01.11.20', null, true);
update versicherungen set Aktiv = false, Kündigungsdatum = '31.10.20' where Typ = 'KFZ' and vid= 3;
select sum(Zahlbetrag) as 'jährliche Kosten' from versicherungen; 
UPDATE versicherungen set zahlbetrag = 'quartal', Zahlbetrag = 3*zahlbetrag - 3*zahlbetrag*0.05 where Versicherung = 'Generali';
...
insert into filmliste (FilmID, Titel,Beschreibung, Kategorie, Regie, Darsteller, Jahr, dauer, Altersfreigabe)
values (1001, 'Der Herr der Ringe', 'Zusammen mit seinen mutigen ....', 'Fantasy', 'Peter Jackson', 'Elijah Wood, Ian McKellen, Liv Tyler, Viggo Mortensen', '2002', 183, 12),
(1002, 'Armageddon', 'Ein Asteroid...', 'Action', 'Micheal Bay', 'Charlston Heston, Steve Buscemi, William Fichtner, Micheal Clarke Duncan', '1998', 148, 12),
(1003, 'Jennifer 8', 'Ein schrecklicher Fund...', 'Krime', 'Bruce Robinson', 'Andy Garcia, Lance Henrikson, Uma Thurman, Kathy Baker', '1992', 120, 16),
(1004, 'Sieben', 'Eine unheimlich Mordserie...', 'Krime', 'David Fincher', 'Morgan Freeman, Brad Pitt, Kevin Spacey', '1995', 127, 16),
(1005, 'Findet Nemo', 'Vater verliert seinen Sohn....', 'Zeichentrick', 'Andrew Stanton, Lee Unkrich', null, '2003', 100, null),
(1006, 'Star Wars-Der letzte Jedi', null, 'SciFi', 'Rian Johnson', 'Daisy Ridley, John Boyega, Mark Hamiil', '2017', null, null),
(1007, 'Alien 2', 'Mensch gegen Alien', 'SciFi', 'James Cameron', 'Sigourney Weaver, Micheal Biehn', '1996', 148, 16);

select Jahr, Titel||' mit Brad Pitt' as 'Titel mit Brad' from Filmliste where Darsteller like '%Brad Pitt%'; 
select Jahr, Titel||' mit Mark Hamill' as 'Titel mit Mark' from Filmliste where Darsteller like '%Mark Hamiil%'; 


select Jahr, Titel||' mit '||
CASE 
when darsteller like '%Brad Pitt%' then 'Brad Pitt'
when Darsteller like '%Mark Hamiil%' then 'Mark Hamiil'
end as 'Titel mit..'
from filmliste where Darsteller like '%Brad Pitt%' or Darsteller like '%Mark Hamiil%';


select * from filmliste where Regie like '%,%' order by Jahr asc;
select round(avg(dauer)) as 'durchschnittliche Spieldauer in minuten' from filmliste;
select jahr, titel, length(Beschreibung) as 'Länge' from filmliste where Jahr BETWEEN '1991' and '2001' order by LENGTH(Beschreibung) desc;

create table ParkplatzListe(
Platzbezeichnung char(5) primary key,
Name varchar(60),
Vorname varchar(60),
...
select 'Parkplatz '||Platzbezeichnung as 'Parkplatz' from ParkplatzListe where Name is null and Vorname is null;
